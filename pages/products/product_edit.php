...
if ($_POST) {
        ...
        $stock = $_POST['stock'];
        $warn_stock = $_POST['warn_stock'];

        $brand = mysqli_real_escape_string($conn, $brand);
        $model = mysqli_real_escape_string($conn, $model);
        $sql = "UPDATE product SET brand='$brand', model='$model', price='$price', cost_price='$cost_price', stock='$stock', warn_stock='$warn_stock' WHERE id=$id";

        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('商品修改成功');location.href='product_list.php';</script>";
        } else {
            echo "<script>alert('修改失败：" . mysqli_error($conn) . "');history.back();</script>";
        }
        exit;
    }
?>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="post">
                        <div class="mb-3">
                            <label>品牌</label>
                            <input type="text" name="brand" class="form-control" value="<?= isset($row['brand']) ? $row['brand'] : '' ?>">
                        </div>
                        <div class="mb-3">
                            <label>型号</label>
                            <input type="text" name="model" class="form-control" value="<?= isset($row['model']) ? $row['model'] : '' ?>">
                        </div>
                        <div class="mb-3">
                            <label>进价（元） <span style="color:#999; font-size:12px;">（采购成本）</span></label>
                            <input type="number" step="0.01" min="0" name="cost_price" class="form-control" value="<?= isset($row['cost_price']) ? $row['cost_price'] : '0.00' ?>" required>
                        </div>
                        <div class="mb-3">
                            <label>单价（元） <span style="color:#999; font-size:12px;">（销售价格）</span></label>
                            <input type="number" step="0.01" min="0.01" name="price" class="form-control" value="<?= $row['price'] ?>" required>
                        </div>
                        <div class="mb-3">
                            <label>库存数量</label>
                            <input type="number" min="0" name="stock" class="form-control" value="<?= $row['stock'] ?>" required>
                        </div>
                        <div class="mb-3">
                            <label>库存预警值</label>
                            <input type="number" min="0" name="warn_stock" class="form-control" value="<?= $row['warn_stock'] ?>" required>
                        </div>
                        <div>
                            <button class="btn btn-primary">保存修改</button>
                            <a href="product_list.php" class="btn btn-outline-secondary ml-2">返回列表</a>
                        </div>
                    </form>
                </div>
            </div>
