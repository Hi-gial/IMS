    ...
    if (file_exists('../../config/install.lock.php')) {
        ...
        if (isset($config['db_host'], $config['db_user'], $config['db_pwd'], $config['db_name'])) {
            ...
            if ($conn) {
                ...
                if ($tables_exist) {
                    $result = mysqli_query($conn, "SHOW COLUMNS FROM product LIKE 'cost_price'");
                    if (mysqli_num_rows($result) == 0) {
                        mysqli_query($conn, "ALTER TABLE product ADD COLUMN cost_price DECIMAL(10,2) DEFAULT 0.00 COMMENT '进价' AFTER price");
                    }
                }
...
// 检查查看权限
if ($_SESSION['role'] != 1 && (!isset($perm['p_index']) || $perm['p_index'] != 1)) {
...
if (isset($_GET['export_csv'])) {
    ...
    $search_model = isset($_GET['search_model']) ? $_GET['search_model'] : '';
    
    $where = array();
    if (!empty($search_brand)) {
        $where[] = "brand LIKE '%$search_brand%'";
    }
    ...
    $sql = "SELECT * FROM product $where_clause ORDER BY id DESC";

    $res = mysqli_query($conn, $sql);

    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header('Content-Disposition: attachment; filename="商品列表_' . date('Ymd') . '.xls"');
    ...
    echo '<tr><td colspan="8" style="text-align: center; font-weight: bold; font-size: 16px; padding: 10px;">康保县城乡发展集团有限公司商品列表</td></tr>';
    echo '<thead><tr style="background-color: #f8f9fa; font-weight: bold;">';
    echo '<th>序号</th><th>品牌</th><th>型号</th><th>进价</th><th>单价</th><th>库存</th><th>金额合计</th><th>预警值</th>';
    echo '</tr></thead>';
    echo '<tbody>';
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        $cost_price = isset($row['cost_price']) ? $row['cost_price'] : 0;
        $stock = $row['stock'];
        $row_amount = $cost_price * $stock;
        $sum_stock += $stock;
        ...
        echo '<td>' . $stock . '</td>';
        echo '<td>¥' . number_format($row_amount, 2) . '</td>';
        echo '<td>' . $row['warn_stock'] . '</td>';
        echo '</tr>';
    }

    echo '<tr style="background-color: #f5f5f5; font-weight: bold;">';
    echo '<td colspan="5" align="center">合计</td>';
    echo '<td>' . $sum_stock . '</td>';
    ...
    <div class="layout-container">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <div class="welcome-info" style="padding: 15px 20px; font-size: 14px; color: #bdc3c7; border-bottom: 1px solid #34495e; margin-bottom: 10px; text-align: center;">
                欢迎：<?= $_SESSION['real_name'] ?>（<?= $is_admin ? '管理员' : '普通用户' ?>）
            </div>
            <a href="../dashboard/index.php" class="menu-item">
                <span class="icon">🏠</span>
                <span class="text">首页</span>
            </a>
            <a href="product_list.php" class="menu-item active">
                <span class="icon">📦</span>
        ...
        <div class="content">
            ...
            <div class="d-flex justify-content-end align-items-center mb-3" style="gap: 8px;">
                <?php if ($is_admin) { ?>
                    <a href="product_add.php" class="btn btn-primary">添加商品</a>
                <?php } ?>
                    <a href="product_list.php?export_csv=1<?= !empty($search_brand) ? '&search_brand=' . urlencode($search_brand) : '' ?><?= !empty($search_model) ? '&search_model=' . urlencode($search_model) : '' ?>" class="btn btn-success">导出表格</a>
            </div>

            <div class="mb-3">
                <div class="d-flex justify-content-end">
                    <form method="GET" class="search-form" style="flex-wrap: wrap; gap: 10px;" onsubmit="clearSearchInputs()">
                        <input type="text" name="search_brand" class="form-control" placeholder="搜索品牌" id="search-brand-input" style="min-width: 150px;">
                        <input type="text" name="search_model" class="form-control" placeholder="搜索型号" id="search-model-input" style="min-width: 150px;">
                        <button type="submit" class="btn btn-primary">搜索</button>
                        <a href="product_list.php" class="btn btn-secondary">取消</a>
                    </form>
                </div>
                <script>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    echo "<script>
                        if (!URLSearchParams && !window.location.search.includes('page_size')) {
                            var savedPageSize = localStorage.getItem('page_size');
                            if (savedPageSize) {
                                var url = new URL(window.location.href);
                                url.searchParams.set('page_size', savedPageSize);
                    ...
                    if ((!empty($search_brand) || !empty($search_model)) && $_SESSION['role'] != 1 && (!isset($perm['p_search']) || $perm['p_search'] != 1)) {
                    ...
                    $count_sql = "SELECT COUNT(*) FROM product $where_clause";
                    $sql = "SELECT * FROM product $where_clause ORDER BY id DESC LIMIT $offset, $page_size";
                    ...
                    $total_sql = "SELECT SUM(stock) AS total_stock, SUM(IFNULL(cost_price, 0) * stock) AS total_amount FROM product $where_clause";
                    $total_res = mysqli_query($conn, $total_sql);
                    $total_data = mysqli_fetch_assoc($total_res);
                    $total_stock = $total_data['total_stock'] ?: 0;
                    ...
                    ?>
                        <tr>
                            ...
                            <td<?= !empty($row['warn_stock']) && $row['warn_stock'] > 0 && $stock <= $row['warn_stock'] ? ' style="background:red; font-weight: bold; color: white;"' : '' ?>><?= $row['warn_stock'] ?></td>
                            <?php if ($is_admin) { ?>
                                <td>
                                    <a href="product_edit.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-primary">编辑</a>
                                    <a href="product_del.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('确定要删除该商品吗？');">删除</a>
                                </td>
                            <?php } ?>
                        </tr>
                    <?php } ?>
                ...
                <div class="d-flex justify-content-end align-items-center">
                    ...
                    <nav class="ml-3">
                        <ul class="pagination">
                            <?php if ($page > 1) { ?>
                                <li>
                                    <a href="?page=1&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&page_size=<?= $page_size ?>">首页</a>
                                </li>
                                <li>
                                    <a href="?page=<?= $page - 1 ?>&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&page_size=<?= $page_size ?>">
                            ...
                            <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++) { ?>
                                <li>
                                    <a href="?page=<?= $i ?>&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&page_size=<?= $page_size ?>" class="<?= $i == $page ? 'btn-primary' : '' ?>"><?= $i ?></a>
                                </li>
                            <?php } ?>
                            ...
                            <?php if ($page < $total_pages) { ?>
                                ...
                                <li>
                                    <a href="?page=<?= $total_pages ?>&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&page_size=<?= $page_size ?>">末页</a>
                                </li>
                            <?php } ?>
                        </ul>
                    </nav>
