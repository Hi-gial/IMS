...
function check_installation() {
    if (file_exists('../../../config/install.lock.php')) {
        ...
        if (isset($config['db_host'], $config['db_user'], $config['db_pwd'], $config['db_name'])) {
            ...
            if ($conn) {
                $result = mysqli_query($conn, "SHOW TABLES LIKE 'receive_unit'");
                if (mysqli_num_rows($result) == 0) {
                    mysqli_query($conn, "CREATE TABLE receive_unit (
...
$count_res = $conn->query("SELECT COUNT(*) FROM receive_unit" . $search_condition);
$count_row = $count_res->fetch_row();
$total = $count_row[0];
$total_pages = ceil($total / $page_size);

$res = $conn->query("SELECT * FROM receive_unit" . $search_condition . " ORDER BY id DESC LIMIT $offset, $page_size");
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <title>捷顺ims - 领取单位管理</title>
...
<body>
    <div class="layout-container">
        <div class="sidebar" id="sidebar">
            ...
            <a href="receive_unit_list.php" class="menu-item active">
                <span class="icon">🏢</span>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (sidebarCollapsed) {
                    ...
                    menuItems.forEach(item => { item.style.textAlign = 'center'; });
        ...
        <div class="content">
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-end">
                    <a href="receive_unit_add.php" class="btn btn-success">添加领取单位</a>
                </div>
            </div>
            <div class="mb-3">
                <form method="GET" class="search-form" style="display: inline-flex !important; flex-wrap: wrap; gap: 10px; align-items: center; width: auto;">
                    <input type="text" name="keyword" class="form-control" placeholder="搜索领取单位名称" style="width: 48%;">
                    <button type="submit" class="btn btn-primary" style="height: 38px;">搜索</button>
                    <a href="receive_unit_list.php" class="btn btn-secondary" style="height: 38px;">取消</a>
                </form>
            </div>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $r['created_at'] ?></td>
                        <td>
                            <a href="receive_unit_edit.php?id=<?=h($r['id'])?>" class="btn btn-sm btn-primary">修改</a>
                            <a href="receive_unit_del.php?id=<?=h($r['id'])?>" class="btn btn-sm btn-danger" onclick="return confirm('确定删除该领取单位？')">删除</a>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
            ...
            <div class="mt-3">
                <div class="d-flex justify-content-end align-items-center">
                    ...
                    <form method="GET" class="ml-3" style="margin-right: 15px;">
                        ...
                        <input type="hidden" name="keyword" value="<?= urlencode($keyword) ?>">
                    ...
                    <nav class="ml-3">
                        <ul class="pagination">
                            <?php if ($page > 1) { ?>
                            <li><a href="?page=1&keyword=<?= urlencode($keyword) ?>&page_size=<?= $page_size ?>">首页</a></li>
                            <li><a href="?page=<?= $page - 1 ?>&keyword=<?= urlencode($keyword) ?>&page_size=<?= $page_size ?>">上一页</a></li>
                            <?php } ?>

                            <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++) { ?>
                            <li><a href="?page=<?= $i ?>&keyword=<?= urlencode($keyword) ?>&page_size=<?= $page_size ?>" class="<?= $i == $page ? 'btn-primary' : '' ?>"><?= $i ?></a></li>
                            <?php } ?>

                            <?php if ($page < $total_pages) { ?>
                            <li><a href="?page=<?= $page + 1 ?>&keyword=<?= urlencode($keyword) ?>&page_size=<?= $page_size ?>">下一页</a></li>
                            <li><a href="?page=<?= $total_pages ?>&keyword=<?= urlencode($keyword) ?>&page_size=<?= $page_size ?>">末页</a></li>
                            <?php } ?>
                        </ul>
                    </nav>
                </div>
