...
if ($_POST) {
    ...
    $stmt = mysqli_prepare($conn, "INSERT INTO supplier (name, contact, phone, remark) VALUES (?, ?, ?, ?)");
    if (!$stmt) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        write_log($conn, $_SESSION['username'], "添加供货商：$name");
        header("location:supplier_list.php");
        exit;
    } else {
        echo "<script>alert('添加供货商失败：" . mysqli_error($conn) . "');history.back();</script>";
        exit;
    }
}
?>
                ...
                <div class="card-body">
                    <form method="post">
                        <div class="mb-3">
                            ...
                            <input name="name" class="form-control" required placeholder="请输入供货商名称">
                        </div>
                        <div class="mb-3">
