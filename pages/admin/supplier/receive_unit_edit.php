...
$stmt = mysqli_prepare($conn, "SELECT * FROM receive_unit WHERE id = ?");
...
if ($_POST) {
    ...
    if (empty($name)) {
        echo "<script>alert('请输入领取单位名称');history.back();</script>";
        exit;
    }

    $stmt = mysqli_prepare($conn, "UPDATE receive_unit SET name = ?, contact = ?, phone = ?, remark = ? WHERE id = ?");
    if (!$stmt) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        write_log($conn, $_SESSION['username'], "修改领取单位：$name");
        echo "<script>alert('保存成功');location.href='receive_unit_list.php';</script>";
        exit;
    } else {
        echo "<script>alert('保存失败：" . mysqli_error($conn) . "');history.back();</script>";
        exit;
    }
}
?>
        ...
        <div class="content">
            ...
            <div class="card w-50">
                <div class="card-body">
                    <form method="post">
                        <div class="mb-3">
                            <label class="form-label">领取单位名称</label>
                            <input name="name" class="form-control" value="<?=h($unit['name'])?>" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">联系人</label>
                            <input name="contact" class="form-control" value="<?=h($unit['contact'])?>" placeholder="请输入联系人（选填）">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">联系电话</label>
                            <input name="phone" class="form-control" value="<?=h($unit['phone'])?>" placeholder="请输入联系电话（选填）">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">备注</label>
                            <textarea name="remark" class="form-control" rows="3" placeholder="请输入备注信息（选填）"><?=h($unit['remark'])?></textarea>
                        </div>
                        <div>
                            <button class="btn btn-success">保存</button>
                            <a href="receive_unit_list.php" class="btn btn-outline-secondary ml-2">返回</a>
                        </div>
                    </form>
                </div>
            </div>
