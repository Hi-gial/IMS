...
if ($_POST) {
    ...
    $stmt = mysqli_prepare($conn, "INSERT INTO receive_unit (name, contact, phone, remark) VALUES (?, ?, ?, ?)");
    if (!$stmt) {
        echo "<script>alert('数据库错误：" . mysqli_error($conn) . "');history.back();</script>";
        exit;
    }
    mysqli_stmt_bind_param($stmt, "ssss", $name, $contact, $phone, $remark);
    if (mysqli_stmt_execute($stmt)) {
        write_log($conn, $_SESSION['username'], "添加领取单位：$name");
        header("location:receive_unit_list.php");
        exit;
    } else {
        echo "<script>alert('添加领取单位失败：" . mysqli_error($conn) . "');history.back();</script>";
        exit;
    }
}
?>
                ...
                <div class="card-body">
                    <form method="post">
                        <div class="mb-3">
                            ...
                            <input name="name" class="form-control" required placeholder="请输入领取单位名称">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">联系人</label>
                            <input name="contact" class="form-control" placeholder="请输入联系人（选填）">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">联系电话</label>
                            <input name="phone" class="form-control" placeholder="请输入联系电话（选填）">
                        </div>
                        <div class="mb-3">
