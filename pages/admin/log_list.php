...
// XLS导出功能（HTML表格形式，Excel可识别，支持合并单元格）
if (isset($_GET['export_csv'])) {
    $res = $conn->query("SELECT l.*, a.nickname FROM log l LEFT JOIN admin a ON l.username = a.username ORDER BY l.id DESC");

    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header('Content-Disposition: attachment; filename="操作日志_' . date('Ymd') . '.xls"');
    ...
    echo '<tr><td colspan="4" style="text-align: center; font-weight: bold; font-size: 16px; padding: 10px;">康保县城乡发展集团有限公司操作日志</td></tr>';
    echo '<thead><tr style="background-color: #f8f9fa; font-weight: bold;">';
    echo '<th>序号</th><th>操作人</th><th>内容</th><th>时间</th>';
    echo '</tr></thead>';
    echo '<tbody>';

    $index = 1;
    while ($row = $res->fetch_assoc()) {
        $display_name = !empty($row['nickname']) ? $row['nickname'] : $row['username'];
        $content = str_replace("用户 {$row['username']}", "用户 {$display_name}", $row['content']);
        echo '<tr>';
        echo '<td>' . $index++ . '</td>';
...
// 获取总记录数
$count_res = $conn->query("SELECT COUNT(*) FROM log l LEFT JOIN admin a ON l.username = a.username");
$count_row = $count_res->fetch_row();
$total = $count_row[0];
$total_pages = ceil($total / $page_size);

$res = $conn->query("SELECT l.*, a.nickname FROM log l LEFT JOIN admin a ON l.username = a.username ORDER BY l.id DESC LIMIT $offset, $page_size");
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <title>捷顺ims - 日志查看</title>
...
<body>
    <div class="layout-container">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <a href="log_list.php" class="menu-item active">
                <span class="icon">📊</span>
        ...
        <div class="content">
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-end">
                    <a href="log_list.php?export_csv=1" class="btn btn-success">导出表格</a>
                </div>
            </div>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php $index = ($page - 1) * $page_size + 1; while($r=$res->fetch_assoc()){ 
                        $display_name = !empty($r['nickname']) ? $r['nickname'] : $r['username'];
                        $content = str_replace("用户 {$r['username']}", "用户 {$display_name}", $r['content']);
                    ?>
                    <tr>
                        <td><?= $index++ ?></td>
                    ...
                    <nav class="ml-3">
                        <ul class="pagination" style="display: flex; list-style: none; padding: 0; margin: 0;">
                            ...
                            <li style="margin-right: 5px;">
                                <a href="?page=1&page_size=<?= $page_size ?>" style="display: inline-flex; align-items: center; justify-content: center; padding: 6px 12px; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; color: #3498db; background-color: white; transition: all 0.3s ease; min-width: 36px; height: 32px; box-sizing: border-box;">首页</a>
                            </li>
                            <li style="margin-right: 5px;">
                                <a href="?page=<?= $page - 1 ?>&page_size=<?= $page_size ?>" style="display: inline-flex; align-items: center; justify-content: center; padding: 6px 12px; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; color: #3498db; background-color: white; transition: all 0.3s ease; min-width: 36px; height: 32px; box-sizing: border-box;">
                            ...
                            <li style="margin-right: 5px;">
                                <a href="?page=<?= $i ?>&page_size=<?= $page_size ?>" style="display: inline-flex; align-items: center; justify-content: center; padding: 6px 12px; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; color: #3498db; background-color: white; transition: all 0.3s ease; min-width: 36px; height: 32px; box-sizing: border-box;<?= $i == $page ? ' background-color: #3498db; color: white; border-color: #3498db;' : '' ?>"><?= $i ?></a>
                            </li>
                            <?php } ?>
                            ...
                            <li>
                                <a href="?page=<?= $total_pages ?>&page_size=<?= $page_size ?>" style="display: inline-flex; align-items: center; justify-content: center; padding: 6px 12px; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; color: #3498db; background-color: white; transition: all 0.3s ease; min-width: 36px; height: 32px; box-sizing: border-box;">末页</a>
                            </li>
                            <?php } ?>
                        </ul>
                    </nav>
