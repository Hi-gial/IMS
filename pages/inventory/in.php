        ...
        if (isset($config['db_host'], $config['db_user'], $config['db_pwd'], $config['db_name'])) {
            ...
            if ($conn) {
                ...
                if ($tables_exist) {
                    ...
                    $result = mysqli_query($conn, "SHOW COLUMNS FROM stock_in LIKE 'unit'");
                    if (mysqli_num_rows($result) == 0) {
                        mysqli_query($conn, "ALTER TABLE stock_in ADD COLUMN unit VARCHAR(20) DEFAULT NULL COMMENT '单位' AFTER num");
                    }
...
if ($_POST) {
    ...
    if (!is_null($supplier_id)) {
        ...
        $supplier_res = mysqli_stmt_get_result($stmt);
        if ($supplier = mysqli_fetch_assoc($supplier_res)) {
            $supplier_name = $supplier['name'];
        }
    }

    if ($product_id <= 0 || $num <= 0) {
        echo "<script>alert('请输入有效的入库数量');history.back();</script>";
        exit;
    }
    
    if (empty($unit)) {
        echo "<script>alert('请选择入库单位');history.back();</script>";
        exit;
    }
    ...
    mysqli_stmt_execute($stmt);
    $product_res = mysqli_stmt_get_result($stmt);
    if (!mysqli_fetch_assoc($product_res)) {
        echo "<script>alert('该商品不存在');history.back();</script>";
        exit;
    }

    // 获取商品品牌和型号
    $stmt = mysqli_prepare($conn, "SELECT brand, model FROM product WHERE id = ?");
    if (!$stmt) {
        echo "<script>alert('数据库错误：" . mysqli_error($conn) . "');history.back();</script>";
        exit;
    }
    ...
    // 写入入库记录
    if (is_null($supplier_id)) {
        $stmt = mysqli_prepare($conn, "INSERT INTO stock_in (pid, num, unit, operator, supplier_id, supplier_name, remark) VALUES (?, ?, ?, ?, NULL, ?, ?)");
        if (!$stmt) {
            $error = true;
            $error_msg = "数据库错误：" . mysqli_error($conn);
        } else {
            mysqli_stmt_bind_param($stmt, "iissss", $product_id, $num, $unit, $user, $supplier_name, $remark);
            if (!mysqli_stmt_execute($stmt)) {
                $error = true;
                $error_msg = "入库记录失败：" . mysqli_error($conn);
            }
        }
    } else {
        $stmt = mysqli_prepare($conn, "INSERT INTO stock_in (pid, num, unit, operator, supplier_id, supplier_name, remark) VALUES (?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt) {
            $error = true;
            $error_msg = "数据库错误：" . mysqli_error($conn);
        } else {
            mysqli_stmt_bind_param($stmt, "iississ", $product_id, $num, $unit, $user, $supplier_id, $supplier_name, $remark);
            if (!mysqli_stmt_execute($stmt)) {
                $error = true;
                $error_msg = "入库记录失败：" . mysqli_error($conn);
            }
    ...
    // 更新库存
    if (!$error) {
        $stmt = mysqli_prepare($conn, "UPDATE product SET stock = stock + ? WHERE id = ?");
        if (!$stmt) {
            $error = true;
            $error_msg = "数据库错误：" . mysqli_error($conn);
        } else {
            mysqli_stmt_bind_param($stmt, "ii", $num, $product_id);
            if (!mysqli_stmt_execute($stmt)) {
                $error = true;
                $error_msg = "库存更新失败：" . mysqli_error($conn);
            }
        }
    }
    ...
    // 记录操作日志
    if (!$error) {
        $log_content = "用户 {$user} 入库商品：{$product_name}，数量：{$num}";
        $stmt = mysqli_prepare($conn, "INSERT INTO log (username, content) VALUES (?, ?)");
        if (!$stmt) {
            $error = true;
            $error_msg = "数据库错误：" . mysqli_error($conn);
        } else {
            mysqli_stmt_bind_param($stmt, "ss", $user, $log_content);
            if (!mysqli_stmt_execute($stmt)) {
                $error = true;
                $error_msg = "日志记录失败：" . mysqli_error($conn);
            }
        }
    }
    ...
    if ($error) {
        ...
        mysqli_commit($conn);
        echo "<script>alert('入库成功');location.href='in_list.php';</script>";
        exit;
    }
}
?>
...
<body>
    <div class="layout-container">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <!-- 所有人可见 -->
            <a href="in.php" class="menu-item active">
                <span class="icon">📥</span>
        ...
        <div class="content">
            ...
            <div class="card w-50">
                <form method="post">
                    ...
                    <div class="mb-3">
                        <label>入库数量</label>
                        <input type="number" min="1" name="num" class="form-control" placeholder="请输入入库数量" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="supplier_id" class="form-select">
                            ...
                            <?php
                            $supplier_res = mysqli_query($conn, "SELECT * FROM supplier ORDER BY name");
                            while ($supplier = mysqli_fetch_assoc($supplier_res)) {
                    ...
                    <div>
                        <button class="btn btn-primary">确认入库</button>
                        <a href="in_list.php" class="btn btn-warning" style="margin-left: 10px;">查看入库记录</a>
                    </div>
                </form>
            </div>
        </div>
