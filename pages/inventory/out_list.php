...
// 检查查看权限
if ($_SESSION['role'] != 1 && (!isset($perm['p_out_list']) || $perm['p_out_list'] != 1)) {
...
if (isset($_GET['export_csv'])) {
    ...
    $receive_unit = isset($_GET['receive_unit']) ? mysqli_real_escape_string($conn, $_GET['receive_unit']) : '';
    ...
    if (!empty($operator)) {
        $where[] = "(a.nickname LIKE '%$operator%' OR so.operator LIKE '%$operator%')";
    }
    if (!empty($receive_unit)) {
        $where[] = "(so.receive_unit LIKE '%$receive_unit%' OR so.receive_unit_name LIKE '%$receive_unit%')";
    }
    if (!empty($start_date)) {
        $where[] = "so.ctime >= '$start_date 00:00:00'";
    }
    if (!empty($end_date)) {
        $where[] = "so.ctime <= '$end_date 23:59:59'";
    }
    ...
    $sql = "SELECT so.*, p.brand, p.model, a.nickname FROM stock_out so LEFT JOIN product p ON so.pid = p.id LEFT JOIN admin a ON so.operator = a.username $where_clause ORDER BY so.id DESC";
    $res = mysqli_query($conn, $sql);

    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header('Content-Disposition: attachment; filename="出库记录_' . date('Ymd') . '.xls"');
    ...
    echo '<tr><td colspan="9" style="text-align: center; font-weight: bold; font-size: 16px; padding: 10px;">康保县城乡发展集团有限公司出库记录</td></tr>';
    echo '<thead><tr style="background-color: #f8f9fa; font-weight: bold;">';
    echo '<th>序号</th><th>品牌</th><th>型号</th><th>出库数量</th><th>单位</th><th>领取单位</th><th>备注</th><th>操作人</th><th>出库时间</th>';
    echo '</tr></thead>';
    echo '<tbody>';
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo '<td>' . htmlspecialchars($row['receive_unit_name'] ?: $row['receive_unit'] ?: '-') . '</td>';
...
<body>
    <div class="layout-container">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <a href="out_list.php" class="menu-item active">
                <span class="icon">📄</span>
        ...
        <div class="content">
            ...
            if ((!empty($search_brand) || !empty($search_model) || !empty($operator) || !empty($receive_unit) || !empty($start_date) || !empty($end_date)) && $_SESSION['role'] != 1 && (!isset($perm['p_search']) || $perm['p_search'] != 1)) {
            ...
            $count_sql = "SELECT COUNT(*) FROM stock_out so LEFT JOIN product p ON so.pid = p.id LEFT JOIN admin a ON so.operator = a.username $where_clause";
            $sql = "SELECT so.*, p.brand, p.model, a.nickname FROM stock_out so LEFT JOIN product p ON so.pid = p.id LEFT JOIN admin a ON so.operator = a.username $where_clause ORDER BY so.id DESC LIMIT $offset, $page_size";
            ...
            if (!$res) {
                ...
                <div class='mb-3'>
                    <form method='GET' class='search-form' style='display: flex; flex-wrap: wrap; gap: 10px; align-items: end;' onsubmit='clearSearchInputs()'>
                        ...
                        <input type='text' name='receive_unit' class='form-control' placeholder='搜索领取单位' id='receive-unit-input' style='flex: 1; min-width: 150px;'>
                        ...
                        <a href='out_list.php' class='btn btn-secondary' style='height: 38px;'>取消</a>
                    </form>
                    <script>
                        function clearSearchInputs() {
                            setTimeout(function() {
                                ...
                                document.getElementById('receive-unit-input').value = '';
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $row['num'] ?></td>
                        <td><?= $row['unit'] ?: '-' ?></td>
                        <td><?= $row['receive_unit_name'] ?: $row['receive_unit'] ?: '-' ?></td>
                        <td><?= $row['remark'] ?: '-' ?></td>
                    ...
                    <?php 
                        ...
                        if (mysqli_num_rows($res) == 0) {
                            echo "<tr><td colspan='9' style='text-align:center;'>暂无出库记录</td></tr>";
                        }
                    }
                    ?>
                </tbody>
            ...
            <div class="mt-3">
                ...
                <div class="d-flex justify-content-end align-items-center">
                    ...
                    <form method="GET" class="ml-3" style="margin-right: 15px;">
                        ...
                        <input type="hidden" name="receive_unit" value="<?= urlencode($receive_unit) ?>">
                    ...
                    <nav class="ml-3">
                        <ul class="pagination">
                            <?php if ($page > 1) { ?>
                            <li>
                                <a href="?page=1&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&operator=<?= urlencode($operator) ?>&receive_unit=<?= urlencode($receive_unit) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&page_size=<?= $page_size ?>">首页</a>
                            </li>
                            <li>
                                <a href="?page=<?= $page - 1 ?>&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&operator=<?= urlencode($operator) ?>&receive_unit=<?= urlencode($receive_unit) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&page_size=<?= $page_size ?>">
                            ...
                            <li>
                                <a href="?page=<?= $i ?>&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&operator=<?= urlencode($operator) ?>&receive_unit=<?= urlencode($receive_unit) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&page_size=<?= $page_size ?>" class="<?= $i == $page ? 'btn-primary' : '' ?>"><?= $i ?></a>
                            </li>
                            <?php } ?>
                            ...
                            <li>
                                <a href="?page=<?= $total_pages ?>&search_brand=<?= urlencode($search_brand) ?>&search_model=<?= urlencode($search_model) ?>&operator=<?= urlencode($operator) ?>&receive_unit=<?= urlencode($receive_unit) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&page_size=<?= $page_size ?>">末页</a>
                            </li>
                            <?php } ?>
                        </ul>
                    </nav>
